...
import { formatAccess, formatCompatibility } from '../lib/spot-utils';
  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
      <div className="flex items-start justify-between gap-4">
        ...
        <Pill
          tone={
            point.compatibility === 'confirmed-220v'
              ? 'emerald'
              : point.compatibility === 'possible-220v'
                ? 'sky'
                : point.compatibility === 'car-only'
                  ? 'amber'
                  : 'rose'
          }
        >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Pill>{formatAccess(point.access)}</Pill>
        <Pill tone="slate">
